			...
			connection.query('select brandNo, brandName, brandContent, brandImageName image from brand', function(err, result, field) {
				...
				connection.query('select brandNo from brandLikeList where userNo=?', [req.session.userNo], function(err, result, field) {
					...
					connection.query('select productNo, productName, productPrice from productlist', function(err, result, field) {
						...
						var product = result;
						connection.query('select communityNo, email, brandName, brandImageName, image, view, community.content, count(commentNo) count from community join user on community.userNo = user.userNo join brand on community.brandNo = brand.brandNo join communityComment on community.communityNo = communityComment.communityNo order by communityNo desc', function(err, result, field) {
							if (err) {
								console.error(err);
								return;
							}
